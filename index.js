...
// cumleKur.birinci : "Ben";
// cumleKur.ikinci = "iyi";
// cumleKur.ucuncu = "bir";
// cumleKur.dorduncu = "yazılımcı";
// cumleKur.besinci = "olacağım!";
...
function cumlelereDonustur(cumleler, ayrac =",") {
 let result= cumleler.map((cumle) => cumle.join(ayrac));
 return result
}
...
function paragrafOlustur(array,cumleKur,cumlelereDonustur) {
  let dizi= cumlelereDonustur(array, " ")
  let paragraf = 0

  paragraf = cumleKur(dizi[1], dizi[3], dizi[5], dizi[7], dizi[9]);
...
for( let i =0 ; i<keys.length ; i++) {
  mesaj = mesaj.replaceAll(keys[i].toUpperCase(), emojiler[keys[i]])
  mesaj = mesaj.replaceAll(keys[i].toLowerCase(), emojiler[keys[i]])
}

return mesaj
}
//let mesaj = mesaj.replaceAll{}

console.log(emojileriDonustur("Bugun keyfım yerınde :D :d :) ", emojiler));
